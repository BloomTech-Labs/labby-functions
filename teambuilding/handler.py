...
def __get_best_assignment() -> AssignmentTuple:
    ...
    print("Finding best assignment for {} projects and {} students".format(len(projects), len(students)))
    for project in projects:
        for student in students:
            print("==================================================================")
            ...
            if score > -5000:
                if best_assignment.project is None:
                    print("Starting high score: {} - {} = {}\n".format(project_name, student_name, score))
...
def __get_score(project: dict, student: dict) -> int:
    ...
    if student_track not in project_tracks:
        print("Student {} track not needed in project {}".format(student_name, project_name))
    ...
    project_type_preference_score = __calculate_project_type_preference_score(project, student)
    print("Adding project type preference score: {}".format(project_type_preference_score))
    ...
    web_focus_score = __calculate_score_for_average_goal(project, student, WEB_TIER_FOCUS_FIELD, 3.00, 5)
    ...
    web_data_modeling_score = __calculate_score_for_average_goal(project, student, WEB_DATA_MODELING_FIELD, 3.00, 5)
    ...
    web_design_score = __calculate_score_for_average_goal(project, student, WEB_DESIGN_FIELD, 3.00, 5)
    ...
    data_model_confidence_score = __calculate_score_for_average_goal(project, student, DATA_MODELING_CONFIDENCE, 3.00, 5)
    ...
    assertiveness_score = __calculate_score_for_average_goal(project, student, ASSERTIVENESS, 3.00, 5)
    print("Adding score: {}".format(assertiveness_score))
    score += assertiveness_score

    PLANNING = "Do you plan ahead or play it by ear?"
    print("\n== {} ==".format(PLANNING))
    planning_score = __calculate_score_for_average_goal(project, student, PLANNING, 3.00, 5)
...
def __calculate_team_size_score(project: dict, student: dict) -> int:
    ...
    print("Project {} has {} members in track {} with {} being the average for that track"
          .format(project_name, team_member_count, student_track, average_team_size))
...
def __calculate_score_for_average_goal(project: dict, student: dict, survey_field: str, desired_average: float, weight: int) -> int:
    project_name = project['fields']['Name - Cohort']
    student_name = student['fields']['Student Name'][0]

    print("Calculating score for: Project({}) - Student({}) - Field({}) - Desired Average({})"
          .format(project_name, student_name, survey_field, desired_average))
    ...
    print("Score: {} (Based on team average {} and student response {})".format(score, team_response_average, student_response))
...
def __calculate_team_compatibility_score(project: dict, student: dict) -> int:
    ...
    print("Calculating compatibility score for: Project({}) - Student({})"
          .format(project_name, student_name))
