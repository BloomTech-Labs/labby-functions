...
def notify_interviewees(event, context):
    ...
    for person_record in person_records:
        ...
        if "Interview Scheduled" in person_record["fields"]:
            print(
                "Skipping {} because interview is scheduled".format(
                    person_smt_record_id
                )
            )
            continue
        ...
        if (
            "Item" in person_memory_item
            and "last_interview_notification" in person_memory_item["Item"]
        ):
            time_last_notified_string = person_memory_item["Item"][
                "last_interview_notification"
            ]
            time_last_notified = datetime.datetime.fromisoformat(
                time_last_notified_string
            )

            print("Person last notified: {}".format(time_last_notified))

            minutes_since_notified = (
                datetime.datetime.utcnow() - time_last_notified
            ).total_seconds() // 60
