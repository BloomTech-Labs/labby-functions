...
def remind_students_about_late_peer_reviews(event, context):
    ...
    print("Checking {} students with sprint retros older than {} days".format(len(students), DAYS_LATE_BEFORE_WARNING))
    for student_record in students:
        ...
        print("Processing student {} with Slack User ID {}".format(first_name, slack_id))
        ...
        if(slack_response['ok']):
            ...
            try:
                response = client.chat_postMessage(channel  = '#labby_testing',
                                                   text     = message,
                                                   username = USERNAME,
                                                   icon_url = ICON_URL)
            except SlackApiError as error:
